    ...
    let arrayProductos = customFetch(2000, productos).then(
        data => {
            ...
            for(let i=0; i<data.length; i++) {
                products += `<div class="card border-dark m-auto mt-2 " style="width: 18rem;">
                <img class="card-img-top" alt="..." src="${data[i].thumbnail}">
                <h3 class="card-title text-center">${data[i].title}</h3>
                </div>`;
            }
...
const dibujar = (respuesta) => {
    ...
    respuesta.data.results.forEach(product => {
        products += `
        <div class="card border-dark m-auto mt-2 " style="width: 18rem;">
        <img class="card-img-top" alt="..." src="http://http2.mlstatic.com/D_${product.thumbnail_id}-I.jpg">
